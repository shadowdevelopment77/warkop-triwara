...
export class HppService {
  ...
  private cachedIngredientsMap: Map<number, IIngredient> | null = null;
  private cacheTimestamp: number = 0;
  private readonly CACHE_TTL_MS = 10000; // 10 seconds TTL

  constructor(database: TriwaraDatabase = db) {
    this.database = database;
  }

  /** Clears in-memory ingredients cache */
  public invalidateCache(): void {
    this.cachedIngredientsMap = null;
    this.cacheTimestamp = 0;
  }

  /** Gets or caches the ingredient map */
  private async getIngredientsMap(): Promise<Map<number, IIngredient>> {
    const now = Date.now();
    if (this.cachedIngredientsMap && now - this.cacheTimestamp < this.CACHE_TTL_MS) {
      return this.cachedIngredientsMap;
    }

    const ingredients = await this.database.ingredients.toArray();
    const map = new Map<number, IIngredient>();
    ingredients.forEach((ing) => {
      if (ing.id) map.set(ing.id, ing);
    });

    this.cachedIngredientsMap = map;
    this.cacheTimestamp = now;
    return map;
  }

  /**
   * Synchronously evaluates stock availability for a product using an ingredient map.
   */
  private evaluateProductStockSync(
    product: IProduct,
    ingMap: Map<number, IIngredient>,
    orderType: OrderType,
    requestedQty: number
  ): IAvailabilityInfo {
      ...
      for (const item of product.recipe) {
        ...
        if (!ing || ing.currentStock < needed) {
          return {
            ...
            maxPossibleQty:
              ing && item.amount > 0
                ? Math.floor(Math.max(0, ing.currentStock) / item.amount)
                : 0,
          };
        }
      }
    ...
    if (
      orderType === 'takeaway' &&
      product.takeawayPackaging &&
      Array.isArray(product.takeawayPackaging)
    ) {
  ...
  /**
   * Evaluates stock availability for an entire list of products in a single batch.
   * Runs in < 1ms by querying ingredients ONCE instead of N sequential queries.
   */
  async checkBatchStockAvailability(
    products: IProduct[],
    orderType: OrderType = 'dine_in',
    requestedQty: number = 1
  ): Promise<Record<number, IAvailabilityInfo>> {
    const ingMap = await this.getIngredientsMap();
    const result: Record<number, IAvailabilityInfo> = {};

    for (const product of products) {
      if (!product.id) continue;
      result[product.id] = this.evaluateProductStockSync(product, ingMap, orderType, requestedQty);
    }
  ...
  async checkStockAvailability(
    ...
    const ingMap = await this.getIngredientsMap();
    return this.evaluateProductStockSync(product, ingMap, orderType, requestedQty);
  }
  ...
  async deductInventoryForOrder(order: IOrder): Promise<string[]> {
    ...
    await this.database.transaction(
      'rw',
      [this.database.ingredients, this.database.inventoryLogs, this.database.products],
      async () => {
  ...
  /** Restores inventory when an order is voided (Atomic ACID Transaction) */
